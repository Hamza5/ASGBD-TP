...
\section{Langage de définition de données}
	\begin{enumerate}
		...
		\lstinputlisting[firstline=100, lastline=101, style=OracleSQL]{../Scripts/TP1.sql}
		Résultat : \texttt{Table altered.}
		\item Supprimer et recréer la colonne \texttt{TEL\_EMP} :
		\lstinputlisting[firstline=104, lastline=106, style=OracleSQL]{../Scripts/TP1.sql}
		Résultats :\\
		\texttt{Table altered.}
		\begin{verbatim}
		Name                                      Null?    Type
		...
		\lstinputlisting[firstline=109, lastline=110, style=OracleSQL]{../Scripts/TP1.sql}
		Résultats :\\
		\texttt{Table altered.}
		\begin{verbatim}
		Name                                      Null?    Type
		...
		\lstinputlisting[firstline=113, lastline=113, style=OracleSQL]{../Scripts/TP1.sql}
		Résultat : \texttt{Table altered.}
		\item Ajouter la contrainte qui impose que chaque médecin doit avoir une spécialité :
		\lstinputlisting[firstline=116, lastline=116, style=OracleSQL]{../Scripts/TP1.sql}
		Résultat : \texttt{Table altered.}
	\end{enumerate}
	\vspace{1em}
\section{Langage de manipulation de données}
	\begin{enumerate}
		...
		\end{enumerate}
		\lstinputlisting[firstline=558, lastline=561, style=OracleSQL, extendedchars=true]{../Scripts/TP1.sql}
		\item Diminuer 5000 du salaire :
		\lstinputlisting[firstline=564, lastline=564, style=OracleSQL, extendedchars=true]{../Scripts/TP1.sql}
		Resultat :
		\begin{verbatim}
		...
		*
		...
		\lstinputlisting[firstline=565, lastline=565, style=OracleSQL, extendedchars=true]{../Scripts/TP1.sql}
		Resultat : \texttt{Table altered.}\\
		Réessayer :
		\lstinputlisting[firstline=564, lastline=564, style=OracleSQL, extendedchars=true]{../Scripts/TP1.sql}
		Resultat : \texttt{14 rows updated.}\\
		Réactiver la contrainte
		\lstinputlisting[firstline=566, lastline=566, style=OracleSQL, extendedchars=true]{../Scripts/TP1.sql}
		Resultat :
		\begin{verbatim}
