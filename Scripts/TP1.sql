...
ALTER TABLE HOSPITALISATION ADD CONSTRAINT U_LIT UNIQUE(CODE_SERVICE,NUM_CHAMBRE,LIT);
...
ALTER TABLE SOIGNE ADD CONSTRAINT FK_NUM_MED_S FOREIGN KEY(NUM_MED) REFERENCES MEDECIN(NUM_MED) ON DELETE SET NULL;
ALTER TABLE SOIGNE ADD CONSTRAINT FK_NUM_PATIENT_S FOREIGN KEY(NUM_PATIENT) REFERENCES PATIENT(NUM_PATIENT) ON DELETE SET NULL;
/* Les verifications */
...
DESC EMPLOYE;
ALTER TABLE EMPLOYE ADD TEL_EMP NCHAR(10);
...
INSERT INTO SOIGNE VALUES (26,101);
...
/* Question 14 */
INSERT INTO EMPLOYE VALUES (197,'Murray','Andy','Adresse...','Tel...');
INSERT INTO MEDECIN VALUES (197,'Cardiologue');
UPDATE SERVICE SET DIRECTEUR = 197 WHERE CODE_SERVICE = 'CAR';
COMMIT;
...
/* Question 17 */
CREATE INDEX MEDECIN_SPECIALITE ON MEDECIN(SPECIALITE) TABLESPACE Abbad_tbs;

/* Question 18 */
SELECT NOM_PATIENT, PRENOM_PATIENT FROM PATIENT WHERE MUTUELLE = 'MAAF';

/* Question 19 */
SELECT LIT, HOSPITALISATION.NUM_CHAMBRE, SERVICE.CODE_SERVICE, NOM_PATIENT, PRENOM_PATIENT, MUTUELLE FROM HOSPITALISATION INNER JOIN SERVICE ON HOSPITALISATION.CODE_SERVICE = SERVICE.CODE_SERVICE INNER JOIN PATIENT ON HOSPITALISATION.NUM_PATIENT = PATIENT.NUM_PATIENT WHERE BATIMENT = 'B' AND MUTUELLE LIKE 'MN%';

/* Question 20 */
SELECT SOIGNE.NUM_PATIENT, NOM_PATIENT, PRENOM_PATIENT, COUNT(*) AS NB_MEDECINS FROM SOIGNE INNER JOIN PATIENT ON SOIGNE.NUM_PATIENT = PATIENT.NUM_PATIENT GROUP BY SOIGNE.NUM_PATIENT, NOM_PATIENT, PRENOM_PATIENT HAVING COUNT(*) > 3;

/* Question 21 */
SELECT INFIRMIER.CODE_SERVICE, NOM_SERVICE, AVG(SALAIRE) FROM INFIRMIER INNER JOIN SERVICE ON INFIRMIER.CODE_SERVICE = SERVICE.CODE_SERVICE GROUP BY INFIRMIER.CODE_SERVICE, NOM_SERVICE;

/* Question 22 */
SELECT NOM_SERVICE, (SELECT COUNT(*) FROM INFIRMIER WHERE CODE_SERVICE = 'CAR')/(SELECT COUNT(*) FROM HOSPITALISATION WHERE CODE_SERVICE = 'CAR') AS RAPPORT FROM SERVICE WHERE CODE_SERVICE = 'CAR';
SELECT NOM_SERVICE, (SELECT COUNT(*) FROM INFIRMIER WHERE CODE_SERVICE = 'CHG')/(SELECT COUNT(*) FROM HOSPITALISATION WHERE CODE_SERVICE = 'CHG') AS RAPPORT FROM SERVICE WHERE CODE_SERVICE = 'CHG';
SELECT NOM_SERVICE, (SELECT COUNT(*) FROM INFIRMIER WHERE CODE_SERVICE = 'REA')/(SELECT COUNT(*) FROM HOSPITALISATION WHERE CODE_SERVICE = 'REA') AS RAPPORT FROM SERVICE WHERE CODE_SERVICE = 'REA';
